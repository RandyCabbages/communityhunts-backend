...
// ── Tier 1.5: replay a decision the operator already made ───────────────────
//
// WHY THIS EXISTS: Tier 2's apply patches the rows that exist the day it runs, and nothing more.
// The next hunt where a host types the same name produces a fresh id-less row, so the same person
// lands back in the review queue — the queue could never be worked down, only re-worked.
//
// WHY IT IS ALLOWED TO WRITE WITHOUT A PROMPT: the header above says platform-wide alias matching
// must go through human confirmation, and it still does. This does NOT match against the alias
// directory at large. `resolveName` is backed ONLY by aliases whose source is `admin-link` —
// decisions a platform admin already confirmed through Tier 2. Re-asking about a person an
// operator has already ruled on is not a safety rail, it is just noise.
//
// Every other rail is unchanged: blanks only, never an overwrite, never a synthetic id, and a name
// the resolver finds ambiguous returns null and writes nothing.
function linkFromConfirmed(hunt, resolveName) {
  const out = { equity: 0, calls: 0, bonuses: 0, links: [] };
  if (!hunt || typeof resolveName !== 'function') return out;

  const resolve = (name) => {
    if (!norm(name)) return null;
    let id = null;
    try { id = resolveName(name); } catch { return null; }   // a cache miss must never fail a save
    return usableId(id) ? String(id) : null;
  };
  ...
  for (const e of hunt.equity || []) {
    ...
    e.discordId = id;
    out.equity++;
    out.links.push({ kind: 'equity', id: e.id || null, name: e.name, discordId: id });
  }
  for (const c of hunt.calls || []) {
...
module.exports = {
  linkWithinHunt, linkFromConfirmed, proposeFromAliases, collectUnlinkedNames, applyNameLinks,
  unlinkNameLinks,
  // Exported so the confirmed-alias cache keys itself EXACTLY the way rows are grouped here.
  // A second copy of this rule is how the alias directory and the review queue drifted apart.
  normName: norm,
};
