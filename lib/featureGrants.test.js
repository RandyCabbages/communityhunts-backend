...
test('no pgPool → all reads empty, writes no-op', async () => {
  ...
  assert.strictEqual(fg.hasGrant('123', 'shop'), false);
  assert.deepStrictEqual(fg.getGrantsForUser('123'), []);
  await fg.addGrant('123', 'shop', 'admin'); // must not throw
  assert.strictEqual(fg.hasGrant('123', 'shop'), false);
});
...
test('add / has / getGrantsForUser / remove with a DB', async () => {
  ...
  await fg.addGrant('123', 'shop', 'admin');
  assert.strictEqual(fg.hasGrant('123', 'shop'), true);
  assert.deepStrictEqual(fg.getGrantsForUser('123'), ['shop']);
  await fg.removeGrant('123', 'shop');
  assert.strictEqual(fg.hasGrant('123', 'shop'), false);
});

test('init seeds Jeter shop grant', async () => {
  const pool = fakePool();
  await fg.initFeatureGrants({ pgPool: pool });
  assert.strictEqual(fg.hasGrant('461011508713750540', 'shop'), true);
});
