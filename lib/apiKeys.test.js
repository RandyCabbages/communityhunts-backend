...
test('generate → lookup round trip; prefix masks the middle', async () => {
  ...
  assert.deepStrictEqual(await apiKeys.lookupByRawKey(rawKey), { slug: 'acme', scopes: ['read'] });
  assert.strictEqual(await apiKeys.lookupByRawKey('ch_live_wrong'), null);
});

test('generate again = roll: old key stops working, new key works', async () => {
  const first = apiKeys.generateKey('roller', 'u1').rawKey;
  const second = apiKeys.generateKey('roller', 'u1').rawKey;
  assert.notStrictEqual(first, second);
  assert.strictEqual(await apiKeys.lookupByRawKey(first), null);
  assert.deepStrictEqual(await apiKeys.lookupByRawKey(second), { slug: 'roller', scopes: ['read'] });
});

test('revoke deletes the key; lookup then fails; getKeyMeta null', async () => {
...
test('initApiKeys boot-load (fake pgPool): populates mem with ISO-string timestamps; lookupByRawKey + getKeyMeta resolve', async () => {
  ...
  // A row seeded before the scopes column existed reads back as read-only, never unscoped.
  assert.deepStrictEqual(await apiKeys.lookupByRawKey(seededRawKey), { slug: 'pgseeded', scopes: ['read'] });
