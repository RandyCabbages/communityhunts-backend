...
async function parseWinners(tenant, type) {
  ...
  for (let i = 0; i < Math.min(3, messages.length); i++) {
    const m = messages[i];
    console.log(`[discord] msg[${i}] content(${m.content?.length || 0}):`, JSON.stringify(m.content?.slice(0, 300)));
    console.log(`[discord] msg[${i}] embeds(${m.embeds?.length || 0}):`, JSON.stringify((m.embeds || []).map(e => ({ title: e.title, descLen: e.description?.length, fields: e.fields?.length }))));
  }

  // Find the most recent message containing winner results
  // Supports both "Checked-In" format and "Winner: Yes" format
  ...
  if (!resultsMsg) {
    console.log(`[discord] No results msg found. Total messages: ${messages.length}`);
    return { winners: [], count: 0, raw: 'No results message found in last 50 messages' };
  }
  console.log(`[discord] Found results msg, fullText:`, getText(resultsMsg).slice(0, 500));
