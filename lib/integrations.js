...
async function fetchDiscordMessages(botToken, channelId, limit = 100) {
  ...
  console.log(`[discord] fetchMessages: token=${botToken ? botToken.slice(0,10) + '…(' + botToken.length + ' chars)' : '(empty)'} channel=${channelId}`);
...
async function parseWinners(tenant, type) {
  ...
  // Strategy 1: find a message with the formatted #1 table (legacy bots)
  const resultsMsg = messages.find(m => {
    const t = getText(m);
    return t.includes('#1') && (t.includes('Checked-In') || t.includes('Winner'));
  });

  const winners = [];

  if (resultsMsg) {
    const fullText = getText(resultsMsg);
  ...
  // Strategy 2: if no table found, look for a message with @mention pings
  // (bot pings winners with <@userId>, Discord resolves them in mentions array)
  if (!winners.length) {
    const mentionMsg = messages.find(m => (m.mentions?.length >= 2) && /<@\d+>/.test(m.content));
    if (mentionMsg) {
      console.log(`[discord] Found mention-based winners msg: ${mentionMsg.mentions.length} mentions`);
      let place = 1;
      for (const u of mentionMsg.mentions) {
        const name = u.member?.nick || u.global_name || u.username;
        if (name) winners.push({ place: place++, name });
      }
    }
  }

  if (!winners.length) {
    console.log(`[discord] No winners found. Total messages: ${messages.length}, first content: ${messages[0]?.content?.slice(0, 200)}`);
  } else {
    console.log(`[discord] Parsed ${winners.length} winners, first: ${winners[0]?.name}`);
  }
