...
async function parseWinners(tenant, type) {
  ...
  // Debug: dump structure of first 5 messages to find where the results board lives
  for (let i = 0; i < Math.min(5, messages.length); i++) {
    const m = messages[i];
    console.log(`[discord] msg[${i}] author=${m.author?.username} bot=${m.author?.bot} content(${m.content?.length||0}): ${JSON.stringify((m.content||'').slice(0,200))}`);
    if (m.embeds?.length) {
      for (let j = 0; j < m.embeds.length; j++) {
        const e = m.embeds[j];
        console.log(`[discord] msg[${i}].embed[${j}] title=${JSON.stringify(e.title)} desc(${e.description?.length||0}): ${JSON.stringify((e.description||'').slice(0,300))}`);
        if (e.fields?.length) console.log(`[discord] msg[${i}].embed[${j}] fields: ${JSON.stringify(e.fields.map(f=>({name:f.name,val:f.value?.slice(0,100)})))}`);
      }
    }
  }

  return { winners: [], count: 0, raw: 'DEBUG: check Railway logs for message structure' };
