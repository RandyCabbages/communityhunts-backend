...
// Both the immediate call and the interval need .catch: this polls the Twitch API every 5
// minutes, so a Twitch outage or a 429 rejects on a detached promise. That used to be enough
// to take the entire backend down (Node kills the process on an unhandled rejection); it is
// now survivable, but a named log is what makes it diagnosable. Twitch being down must never
// be more than a stale live badge.
function pollTenant(io, tenant) {
  return Promise.resolve(checkTenantLive(io, tenant))
    .catch(e => console.error(`[twitch] live check failed for ${tenant.twitchChannel}:`, e.message));
}
function startTenantPolling(io, tenantList) {
  for (const tenant of tenantList) {
    if (!tenant.twitchChannel) continue;
    pollTenant(io, tenant);
    setInterval(() => pollTenant(io, tenant), 5 * 60 * 1000);
  }
}
...
function pollCreators(io, deps) {
  return Promise.resolve(checkCreatorsLive(io, deps))
    .catch(e => console.error('[twitch] creator live sweep failed:', e.message));
}
function startCreatorPolling(io, deps) {
  pollCreators(io, deps);
  setInterval(() => pollCreators(io, deps), 5 * 60 * 1000);
}
...
async function parseWinners(tenant, type) {
  const channelId = type === 'affiliate'
    ...
    : type === 'toplb'
    ? tenant.discordTopLbWinnersChannelId
