...
function perHunt(h, id) {
  ...
  const huntVault = sumVault(h);         // WHOLE hunt's base-game wins: a winnings/payout figure, NEVER a multiplier numerator
  const hnWinnings = hn + huntVault;     // total won incl. vault = end balance
  ...
  // TWO different vault figures, deliberately. `huntVault` is the whole hunt's base-game wins and
  // stays raw because `hnWinnings` — the hunt's end balance, which the ledger splits — is built
  // from it. The EXPORTED `vault` is this user's SHARE of it, because it is reported next to
  // invested/returned/net, which are all share-adjusted: a member with a 10% stake was reading the
  // whole hunt's $10,000 vault beside their own $700 net.
  // Host owns the whole hunt (share 1); a member gets their share of the hunt's OWN winnings.
  //
  // That share is `natural` (the pre-chase split of totalWon — lib/chaseLedger.js), NOT `returned`.
  // returned is finalPayout, which a post-hunt chase rebuilds from the host-entered rnd.endBalance;
  // that figure includes new deposit money and is in no way bounded by hnWinnings, so a member who
  // chased could be shown MORE vault than the entire hunt banked. The naturals sum to hnWinnings by
  // construction, so this fraction can never exceed 1. With no chase, natural === returned.
  const vault = isOwner
    ? huntVault
    : (hnWinnings > 0 && me ? huntVault * ((Number(me.natural) || 0) / hnWinnings) : 0);
