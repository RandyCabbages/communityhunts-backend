...
test('drops participants marked paid out; empties out a round that was all-paid', () => {
  ...
  const out = sanitizeChases([
    ...
    { id: 'r2', endBalance: 100, participants: [{ id: 'a', deposit: 5 }] }, // only 'a' → all paid → round dropped
  ], payouts);
  assert.strictEqual(out.length, 1);
  assert.strictEqual(out[0].id, 'r1');
  assert.deepStrictEqual(out[0].participants.map(p => p.id), ['b', 'c']); // 'a' (paid) removed
});
