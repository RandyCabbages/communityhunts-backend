...
const MAX_ITEMS = 50;
const states = new Map();    // slug -> { items: [], audio: null, enabled: true }
...
function getState(slug) {
  if (!states.has(slug)) states.set(slug, { items: [], audio: null, enabled: true });
...
// Master switch: OFF = the OBS source page renders nothing (mods stage/arrange items in
// the control panel, invisible to viewers), ON = everything shows on stream. State still
// broadcasts normally either way — only the source page's rendering is gated.
function setEnabled(slug, enabled) {
  const st = getState(slug);
  st.enabled = !!enabled;
  io.to(room(slug)).emit('overdrop:enabled', { enabled: st.enabled });
  return st.enabled;
}

// ── Uploads (mod-pushed media files) ──
// Deliberately on the ephemeral disk: overlay content is transient (in-memory state dies on
// deploy, files die with it) — this is a drop box, not an archive. 24h TTL + count cap keep
// the disk bounded; the sweep runs on each upload (uploads are infrequent).

const UPLOAD_DIR = path.join(os.tmpdir(), 'overdrop-media');
...
const MIME_EXT = {
  ...
  // browsers are sloppy with audio mimes — accept the common aliases
...
function sweepUploads() {
  try {
    const entries = fs.readdirSync(UPLOAD_DIR)
      .map(f => { try { return { f, mtime: fs.statSync(path.join(UPLOAD_DIR, f)).mtimeMs }; } catch (e) { return null; } })
      .filter(Boolean)
      .sort((a, b) => a.mtime - b.mtime); // oldest first
    const now = Date.now();
    let excess = entries.length - (MAX_UPLOAD_FILES - 1);
    for (const { f, mtime } of entries) {
      if (now - mtime > UPLOAD_TTL_MS || excess > 0) {
        try { fs.unlinkSync(path.join(UPLOAD_DIR, f)); } catch (e) { /* already gone */ }
        excess--;
      }
    }
  } catch (e) { /* dir missing = nothing to sweep */ }
}

// Returns { name, kind } or null if the type/size is unacceptable.
function saveUpload(slug, mime, buf) {
  const ext = MIME_EXT[mime];
  if (!ext) return null;
  if (!buf || !Buffer.isBuffer(buf) || !buf.length || buf.length > MAX_UPLOAD_BYTES) return null;
  fs.mkdirSync(UPLOAD_DIR, { recursive: true });
  sweepUploads();
  const name = `${String(slug).replace(/[^a-z0-9_-]/gi, '')}-${Date.now().toString(36)}${Math.random().toString(36).slice(2, 8)}.${ext}`;
  fs.writeFileSync(path.join(UPLOAD_DIR, name), buf);
  return { name, kind: mime.split('/')[0] }; // image | video | audio
}

// Absolute path for serving, or null. Strict name check = no traversal.
function mediaPath(name) {
  if (!/^[a-z0-9_-]+\.[a-z0-9]{2,5}$/i.test(String(name))) return null;
  const p = path.join(UPLOAD_DIR, String(name));
