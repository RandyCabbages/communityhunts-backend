...
test('kpis count live hunts and opened bonuses inside the range', () => {
  ...
  assert.strictEqual(r.kpis.betVolume, 20); // both in-range bonuses' bets, opened or not
});

test('staked sums the starting pot of hunts STARTED in the range', () => {
  const hunts = [
    hunt({ equity: [{ amount: 400 }, { amount: 100 }], startedAt: new Date(NOW - HOUR).toISOString() }),
    hunt({ equity: [{ amount: 900 }], startedAt: new Date(NOW - 100 * HOUR).toISOString() }), // too old
    hunt({ equity: [], startedAt: new Date(NOW - HOUR).toISOString() }),                      // potless
  ];
  const r = computeOverviewMetrics(hunts, { now: NOW, rangeMs: 24 * HOUR, currency: 'USD' });
  assert.strictEqual(r.kpis.staked, 500);
  assert.strictEqual(r.kpis.stakedHunts, 1);   // potless hunts are excluded, not counted as zero

  const all = computeOverviewMetrics(hunts, { now: NOW, rangeMs: null, currency: 'USD' });
  assert.strictEqual(all.kpis.staked, 1400);
  assert.strictEqual(all.kpis.stakedHunts, 2);
});

test('staked buckets by hunt start, NOT by when its bonuses were collected', () => {
  // A hunt funded long ago whose bonuses were all opened in the last hour must not land in Today.
  const hunts = [hunt({
    equity: [{ amount: 250 }],
    startedAt: new Date(NOW - 100 * HOUR).toISOString(),
    bonuses: [{ slot: 'A', bet: 5, win: 50, ts: NOW - HOUR }],
  })];
  const r = computeOverviewMetrics(hunts, { now: NOW, rangeMs: 24 * HOUR, currency: 'USD' });
  assert.strictEqual(r.kpis.staked, 0);
  assert.strictEqual(r.kpis.betVolume, 5);   // the bonus itself IS in range
});

test('staked falls back to createdAt, and keeps undateable hunts in All', () => {
  const hunts = [
    hunt({ equity: [{ amount: 60 }], createdAt: new Date(NOW - HOUR).toISOString() }), // no startedAt
    hunt({ equity: [{ amount: 40 }] }),                                                // no date at all
  ];
  const today = computeOverviewMetrics(hunts, { now: NOW, rangeMs: 24 * HOUR, currency: 'USD' });
  assert.strictEqual(today.kpis.staked, 60);

  const all = computeOverviewMetrics(hunts, { now: NOW, rangeMs: null, currency: 'USD' });
  assert.strictEqual(all.kpis.staked, 100);
});
...
test('all time includes bonuses with no ts; a windowed range still excludes them', () => {
  const hunts = [hunt({ bonuses: [
    { slot: 'Legacy', bet: 7, win: 70 },                    // pre-got-in-log: no ts at all
    { slot: 'Recent', bet: 3, win: 30, ts: NOW - HOUR },
  ] })];
  const all = computeOverviewMetrics(hunts, { now: NOW, rangeMs: null, currency: 'USD' });
  assert.strictEqual(all.kpis.betVolume, 10);
  assert.strictEqual(all.kpis.bonusesOpened, 2);

  // A 24h window cannot place an untimestamped bonus in time, so it must still drop it.
  const today = computeOverviewMetrics(hunts, { now: NOW, rangeMs: 24 * HOUR, currency: 'USD' });
  assert.strictEqual(today.kpis.betVolume, 3);
  assert.strictEqual(today.kpis.bonusesOpened, 1);
});
