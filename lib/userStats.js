...
function computeUserHuntStats(hunts, userId) {
  ...
  for (const h of mine) {
    ...
    let invested, returned;
    if (isOwner) {
      invested = pot; returned = hn;
    } else {
      invested = Number((h.equity || []).find(e => eqUserId(e) === id)?.amount) || 0;
      const share = pot > 0 ? invested / pot : 0;
      returned = hn * share;
    }
    const result = returned - invested;   // net profit/loss, share-adjusted
