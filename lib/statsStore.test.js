...
test('reassignHuntOwner rewrites the stored owner and keeps the huntId key', async () => {
  const pool = fakePool();
  const fxRates = { ensureTable: async () => {}, getUsdRate: async () => 1 };
  const store = makeStatsStore({ pgPool: pool, fxRates });
  await store.recordHunt({
    huntId: 'H1', tenantId: 'bean', user: { id: 'u1', displayName: 'Walker' }, startedAt: 'T0',
    archivedAt: '2026-07-01T00:00:00Z', currency: 'USD',
    equity: [{ id: 'u1', amount: 100 }], bonuses: [{ slot: 'A', bet: 10, win: 40, mult: 4 }],
  });
  const res = await store.reassignHuntOwner('bean', 'H1', { id: 'u2', displayName: 'Mcflurry', avatar: 'a.png' });
  assert.deepStrictEqual(res, { ok: true, huntKey: 'H1' });

  const inserts = pool.calls.filter(c => /INSERT INTO hunt_history/i.test(c.sql));
  const last = inserts[inserts.length - 1];
  assert.strictEqual(last.params[0], 'H1');   // huntId survives the move — same row, upserted
  assert.strictEqual(last.params[2], 'u2');   // host_user_id
  assert.deepStrictEqual(last.params[7].user, { id: 'u2', displayName: 'Mcflurry', avatar: 'a.png' });
  assert.ok(!pool.calls.some(c => /DELETE FROM hunt_history/i.test(c.sql)), 'no rename, nothing to delete');
});

test('reassignHuntOwner renames the row when the hunt has no huntId, deleting the old key', async () => {
  const pool = fakePool();
  const fxRates = { ensureTable: async () => {}, getUsdRate: async () => 1 };
  const store = makeStatsStore({ pgPool: pool, fxRates });
  await store.recordHunt({
    tenantId: 'bean', user: { id: 'u1' }, startedAt: 'T0',   // no huntId => key is user|startedAt
    archivedAt: '2026-07-01T00:00:00Z', currency: 'USD',
    equity: [], bonuses: [{ slot: 'A', bet: 10, win: 40, mult: 4 }],
  });
  const res = await store.reassignHuntOwner('bean', 'u1|T0', { id: 'u2', displayName: 'Mcflurry' });
  assert.deepStrictEqual(res, { ok: true, huntKey: 'u2|T0' });

  const del = pool.calls.find(c => /DELETE FROM hunt_history/i.test(c.sql));
  assert.ok(del, 'the stale key must be deleted or the hunt exists twice, one copy still on the old owner');
  assert.strictEqual(del.params[0], 'u1|T0');
  const inserts = pool.calls.filter(c => /INSERT INTO hunt_history/i.test(c.sql));
  assert.strictEqual(inserts[inserts.length - 1].params[0], 'u2|T0');

  // Write BEFORE delete: a failure between the two must leave a recoverable duplicate, never a
  // hunt that exists in nobody's history.
  const insertAt = pool.calls.findIndex(c => /INSERT INTO hunt_history/i.test(c.sql) && c.params[0] === 'u2|T0');
  const deleteAt = pool.calls.findIndex(c => /DELETE FROM hunt_history/i.test(c.sql));
  assert.ok(insertAt !== -1 && deleteAt > insertAt, 'the new row must be written before the old one is dropped');
});

test('reassignHuntOwner replays the stored FX rate instead of refetching', async () => {
  const pool = fakePool();
  let rate = 0.001;
  const fxRates = { ensureTable: async () => {}, getUsdRate: async () => rate };
  const store = makeStatsStore({ pgPool: pool, fxRates });
  await store.recordHunt({
    huntId: 'H1', tenantId: 'bean', user: { id: 'u1' }, startedAt: 'T0',
    archivedAt: '2026-07-01T00:00:00Z', currency: 'ARS',
    equity: [], bonuses: [{ slot: 'A', bet: 1000, win: 4000, mult: 4 }],
  });
  rate = 999;  // fxRates only knows TODAY's rate — a refetch would restate an old hunt's USD totals
  await store.reassignHuntOwner('bean', 'H1', { id: 'u2', displayName: 'Mcflurry' });
  const inserts = pool.calls.filter(c => /INSERT INTO hunt_history/i.test(c.sql));
  const last = inserts[inserts.length - 1];
  assert.strictEqual(last.params[4], 0.001);
  assert.strictEqual(last.params[7].usdRate, 0.001);
  assert.strictEqual(last.params[7].approx, false, 'a name change must not flag the money approximate');
});

test('reassignHuntOwner returns notFound for a hunt that was never archived', async () => {
  const pool = fakePool();
  const store = makeStatsStore({ pgPool: pool, fxRates: { ensureTable: async () => {}, getUsdRate: async () => 1 } });
  assert.deepStrictEqual(await store.reassignHuntOwner('bean', 'NOPE', { id: 'u2' }), { notFound: true });
});
