...
test('recordHunt stamps equity discordId by name and adds the joined member as a participant', async () => {
  // known_users maps display name "Joiner" -> discord id "u2".
  const pool = fakePool([], [{ user_id: 'u2', display_name: 'Joiner', username: null }]);
  const fxRates = { ensureTable: async () => {}, getUsdRate: async () => 1 };
  const store = makeStatsStore({ pgPool: pool, fxRates });
  await store.recordHunt({
  ...
  // The joiner (resolved to u2 by name) is inserted as a member participant...
  const partU2 = pool.calls.find(c =>
    /INSERT INTO hunt_participants/i.test(c.sql) && c.params[2] === 'u2' && c.params[3] === 'member');
  assert.ok(partU2, 'name-matched joiner u2 added as a member participant');
  ...
  assert.strictEqual(joiner.discordId, 'u2');
});
