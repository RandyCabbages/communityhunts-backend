...
const { sanitizeOverlayConfig, DEFAULT_OVERLAY_CONFIG, persistOverlayConfig } = require('./overlayConfig');
...
test('overlayFill / overlayPattern default to none (overlay stays transparent), gate to enums', () => {
  assert.strictEqual(sanitizeOverlayConfig({}).overlayFill, 'none');
  assert.strictEqual(sanitizeOverlayConfig({}).overlayPattern, 'none');
  assert.strictEqual(sanitizeOverlayConfig({ overlayFill: 'glass' }).overlayFill, 'glass');
  assert.strictEqual(sanitizeOverlayConfig({ overlayFill: 'bogus' }).overlayFill, 'none');
  assert.strictEqual(sanitizeOverlayConfig({ overlayPattern: 'dots' }).overlayPattern, 'dots');
  assert.strictEqual(sanitizeOverlayConfig({ overlayPattern: 'bogus' }).overlayPattern, 'none');
});

test('persistOverlayConfig sanitizes, writes under the key, preserves other settings', async () => {
  const store = { '__mod_hunt__': { preferredSlots: ['x'], overlayConfig: { widgetFill: 'solid' } } };
  const getSettings = async (k) => store[k] || {};
  const saveSettings = async (k, v) => { store[k] = v; };

  const out = await persistOverlayConfig(getSettings, saveSettings, '__mod_hunt__',
    { widgetFill: 'glass', widgetPattern: 'carbon', bogus: 'nope' });

  // returns sanitized config (unknown fields dropped, enums applied)
  assert.strictEqual(out.widgetFill, 'glass');
  assert.strictEqual(out.widgetPattern, 'carbon');
  assert.strictEqual(out.bogus, undefined);
  // persisted under the given key, other settings untouched
  assert.strictEqual(store['__mod_hunt__'].overlayConfig.widgetFill, 'glass');
  assert.deepStrictEqual(store['__mod_hunt__'].preferredSlots, ['x']);
});

test('persistOverlayConfig works when the key has no existing settings', async () => {
  const store = {};
  const getSettings = async (k) => store[k] || {};
  const saveSettings = async (k, v) => { store[k] = v; };
  const out = await persistOverlayConfig(getSettings, saveSettings, '__affiliate_hunt__:t1', { primaryColor: '#a78bfa' });
  assert.strictEqual(out.primaryColor, '#a78bfa');
  assert.strictEqual(store['__affiliate_hunt__:t1'].overlayConfig.primaryColor, '#a78bfa');
});
