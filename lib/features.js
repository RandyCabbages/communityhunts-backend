...
const FEATURES = {
  ...
  // tenant the REQUEST is scoped to. computeFullExtension additionally requires isTenantMember;
...
function computeFullExtension({ isVipHost, isCommunityMod, isDiscordVip, isTenantMember,
  ...
  //
  // isTenantMember is REQUIRED here, not optional politeness. tenantPlan comes from
  ...
  if (isTenantMember && canUse('full_extension', 'free', tenantPlan)) sources.push('partner_plan');
...
//
// Membership is resolved HERE rather than by each caller, deliberately: it is the gate on a paid
// perk, and a caller that forgets to pass it would silently reopen the hole this closes. Every
// lookup fails CLOSED — a DB error means no partner_plan, never a free extension.
async function fullExtensionFor(userId, { tenantPlan, tenantId, isVipHost, isCommunityMod, isDiscordVip } = {}) {
  let subTier = 'free';
  ...
  if (userId && tenantId && memberships) {
    try {
      const mine = await memberships.getUserCommunities(userId);
      isTenantMember = Array.isArray(mine) && mine.map(String).includes(String(tenantId));
    } catch {}
  }
  const hasComp = !!(featureGrants && featureGrants.hasGrant(userId, 'full_extension'));
  return computeFullExtension({
    isVipHost, isCommunityMod, isDiscordVip, isTenantMember, tenantPlan, subTier, hasComp,
