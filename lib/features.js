...
const FEATURES = {
  ...
  // individual — plus a one-time Shop purchase or grandfather grant (see hasFullExtension).
...
// True when the user has it via a plan ladder (Enterprise / Ultimate), a one-time
// Shop purchase (`ext_full` in their cosmeticsOwned), or a grant (grandfather /
// admin comp). `cosmeticsOwned` is passed in by the caller (from user settings).
async function hasFullExtension(userId, tenantPlan, cosmeticsOwned) {
  let tier = 'free';
  if (userId && subscriptions) {
    try { tier = (await subscriptions.getSubscription(userId))?.tier || 'free'; } catch {}
  }
  if (canUse('full_extension', tier, tenantPlan)) return true;
  if (Array.isArray(cosmeticsOwned) && cosmeticsOwned.includes('ext_full')) return true;
  if (featureGrants && featureGrants.hasGrant(userId, 'full_extension')) return true;
  return false;
}
