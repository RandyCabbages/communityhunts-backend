...
// Blank/absent title → a dated default. mod → "Bean's Hunt — Jul 24"; affiliate → "Bean Affiliate Hunt — Jul 24".
function defaultHuntTitle(hostName, kind, now) {
  ...
  return kind === 'affiliate' ? `${host} Affiliate Hunt — ${date}` : `${host}'s Hunt — ${date}`;
}
