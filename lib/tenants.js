...
// 'community_pro'). Feature gating (lib/features.js) ranks the STRIPPED form
// ('starter'/'pro'/'enterprise'), so every tenant object also carries a normalized top-level
// `plan` read via `req.tenant.plan`. A tenant with no plan set falls back to 'pro' (decision
// 2026-07-13): the full community feature set, but NOT the enterprise-only paid extension.
// Mirrors the frontend TenantLayout default — the two must agree or feature gates split-brain
// (UI shows a control the backend then refuses to honor).
function normalizePlan(rawBrandingPlan) {
  const p = String(rawBrandingPlan || '').replace(/^community_/, '');
  return ['free', 'starter', 'pro', 'enterprise'].includes(p) ? p : 'pro';
}
...
module.exports = { PLATFORM_OWNER_ID, PLATFORM_OWNER_IDS, isPlatformOwnerId,
  BEAN_TENANT, initTenants, reloadCache, normalizePlan,
