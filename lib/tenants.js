...
function getTenantDiscordConfig(tenant) {
  ...
  return {
    // Never echo the raw bot token to the client — only whether one is set. The admin form leaves
    // the token field blank ("keep current") and sends a new value only to replace it.
    discordBotTokenSet: !!tenant.discordBotToken,
...
// ── Developer API vocabulary (owner-editable) ──────────────────────────────
// Which hunt types are the community's OWN, and what this community calls each type. Served by
// GET /api/public/v1/me so an integrator reads them instead of hardcoding a guess.
//
// This exists because the alternative was hand-written SQL: the values are per-tenant policy, so
// without an editor every new community needed someone with database access before their
// integrator could get the vocabulary right — the exact opposite of self-serve onboarding.
//
// Validation lives in lib/apiIdentity.js beside the resolvers that READ these, so what saves and
// what publishes cannot disagree. Callers pass an already-sanitized object.
async function updateTenantApiConfig(tenantId, apiConfig) {
  if (!pgPool || !tenantId) return {};
  const clean = (apiConfig && typeof apiConfig === 'object' && !Array.isArray(apiConfig)) ? apiConfig : {};
  await pgPool.query(
    `UPDATE tenants SET branding = jsonb_set(COALESCE(branding, '{}'::jsonb), '{api}', $2::jsonb, true) WHERE id = $1`,
    [tenantId, JSON.stringify(clean)]);
  // Without this the row updates and the change sits inert until the next deploy — the cache is
  // what every read goes through. Same trap documented on updateTenantPlan below.
  await reloadCache();
  return clean;
}
...
// A slug that collides with a static frontend route is UNRECOVERABLE for the community that
// claims it: React Router ranks a static segment above the dynamic `/:slug`, so their community
// is permanently shadowed and the only fix is renaming their slug. Anything that exists as a
// top-level route on the frontend belongs here BEFORE that route ships.
//
// `docs` was added with the public /docs/api page (2026-07-30); `shop`, `support-us` and
// `purchase` had been live as routes for some time without ever being reserved here.
