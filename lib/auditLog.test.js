...
test('summarize: equity.add / equity.remove name the member and the hunt', () => {
  assert.match(
    summarize('equity.add', { actorName: 'Kyle', targetName: "Bean's Hunt", members: [{ name: 'Randy' }] }),
    /Kyle added Randy to Bean's Hunt/);
  assert.match(
    summarize('equity.remove', { actorName: 'Kyle', targetName: "Bean's Hunt", members: [{ name: 'Randy' }] }),
    /Kyle removed Randy from Bean's Hunt/);
});
...
test('recordHuntChange: emits equity.remove alongside a bonus.delete in one PUT', async () => {
  ...
  audit.recordHuntChange(req,
    ...
    { targetId: '__mod_hunt__', targetName: "Bean's Hunt" });
  const r = await audit.query({ limit: 50 });
  ...
  assert.match(eq.summary, /Kyle removed Randy from Bean's Hunt/);
