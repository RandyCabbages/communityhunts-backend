...
//
// 3. THE LISTING ALONE DECIDES REMOVAL; A PLAYABILITY VERDICT CAN ONLY EVER RESCUE.
//    If Rainbet still lists a slug, it stays — full stop — even when the probe cannot get a
//    session out of it. `alive` rescues an entry the listing lost (covering a canonKey gap or
//    listing lag), `unknown` defers, and `dead` is ADVISORY on a listed entry: recorded in
//    rainbet_playability.json and logged, never acted on.
//
//    This is deliberately asymmetric, because the two error directions do not cost the same.
//    A probe failure is one observation from one exit point; a delisting is Rainbet telling
//    us directly. Wrongly keeping a broken game costs a 'no-session' that the extension
//    already detects and handles; wrongly removing a live one deletes a game that no user
//    can reach again until somebody hand-edits the catalogue.
//
//    Availability genuinely does vary by country, and the guard against that is this rule
//    rather than the probe: measured 2026-08-01, six elk-studios games score a raw 'dead'
//    from a US/Iowa exit and all six launch from Comoros. Note the two slugs that started
//    this investigation are NOT that case — avatarux-majestic-meow and voltent-wazdan-bell-wizard
//    fail to launch from Iowa, from Comoros and on a live VPN'd session, while other AvatarUX
//    titles launch from the same exit. They are genuinely broken, they remain listed, and
//    under this rule they are deliberately KEPT. Flipping that is a policy decision, not a
//    bug fix: it means trusting a probe over Rainbet's own catalogue.
function reconcile(entries, live, opts = {}) {
  ...
  const sweptNames = [], markedNames = [], skippedProviders = new Set(), advisoryDead = [];
  ...
  for (const e of entries) {
    ...
    // Listed wins outright; only 'alive' can add to it. A 'dead' verdict never subtracts.
    const isLive = listed || verdict === 'alive';
    if (listed && verdict === 'dead') advisoryDead.push(e.rainbetSlug);
  ...
  return {
    ...
    sweptNames, markedNames, advisoryDead,
