...
test('publicHunt exposes calls but never callerId', () => {
  ...
  assert.deepStrictEqual(out.calls[0], { slot: 'Le Bandit', user: 'Alice', status: 'pending', at: 1720000000000 });
...
test('publicHunt reports the derived category, not the raw huntType', () => {
  ...
  // Mod hunts are stored as huntType 'solo'; the public label must say 'streamer'.
  const mod = { ...HUNT, user: { id: '__mod_hunt__:acme' }, tenantId: 'acme', huntType: 'solo' };
...
test('publicHunt coerces call field values, never emitting a stored object', () => {
  ...
  assert.deepStrictEqual(out.calls[0], { slot: 'Ok', user: null, status: 'pending', at: 5 });
  assert.deepStrictEqual(out.calls[1], { slot: null, user: 'Alice', status: null, at: null });
