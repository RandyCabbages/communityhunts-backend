...
const { huntCompleted, isIdentityMasked } = require('./hunts-core');
...
function hostOperator(details, { cap = TOP_MEMBERS_CAP } = {}) {
  ...
  for (const d of details) {
    ...
    for (const e of (h.equity || [])) {
      ...
      row.hunts++;
      // LAST non-empty display name seen wins. Nothing orders the hunts fed in — statsStore's
      // queries carry no ORDER BY — so this is not "the freshest name", it is whichever row the
      // loop happens to reach last.
    ...
    // The derived category perHunt already computed — re-deriving it here is a second computed
    // value per hunt that can drift from the one every other surface reads.
    const cat = d.category || 'community';
