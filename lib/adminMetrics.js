    ...
    for (const b of h?.bonuses || []) {
      ...
      if (win > 0) {
        ...
        wins.push({
          ...
          // Admin UI links every username to that user's profile; without the id the marquee is
          // the one surface that can't (it falls back to plain text when this is null).
          hunterId: h?.user?.id || null,
        });
      }
    }
  }
