...
// Runner identity is stamped server-side (routes/mod-hunt.routes.js stampRunner) so the hub card
// can show the mod's real Discord avatar, not the community host's letter tile.
test('a stamped runner supplies both the name and the Discord avatar', () => {
  const hunts = {
    __vip_hunt__: {
      user: { id: '__vip_hunt__', displayName: 'Bean', avatar: null }, isLive: true, tenantId: 'bean',
      huntType: 'vip', bonuses: [{ slot: 'x', win: null }], calls: [], equity: [],
      runner: { id: '42', name: 'Mcflury', avatar: 'https://cdn.discordapp.com/avatars/42/abc.png' },
    },
  };
  initHuntsCore({ hunts, archive: [], viewers: {}, io: { to: () => ({ emit: () => {} }) }, persistHunts: () => {} });
  const card = getPublicHunts('bean')[0];
  assert.strictEqual(card.username, 'Mcflury');
  assert.strictEqual(card.avatar, 'https://cdn.discordapp.com/avatars/42/abc.png');
});

// The hub pulls VIP + Affiliate into a featured row. huntType alone cannot do that —
// emptyAffiliateHunt carries huntType 'vip' — so the summary sends the derived category.
test('huntSummary sends the derived category, which huntType cannot supply', () => {
  const mk = (id) => ({
    user: { id }, isLive: true, tenantId: 'bean', huntType: 'vip',
    bonuses: [{ slot: 'x', win: null }], calls: [], equity: [],
  });
  const hunts = { __vip_hunt__: mk('__vip_hunt__'), __affiliate_hunt__: mk('__affiliate_hunt__'), u1: mk('u1') };
  initHuntsCore({ hunts, archive: [], viewers: {}, io: { to: () => ({ emit: () => {} }) }, persistHunts: () => {} });
  const by = Object.fromEntries(getPublicHunts('bean').map(h => [h.userId, h.category]));
  assert.strictEqual(by.__vip_hunt__, 'vip');
  assert.strictEqual(by.__affiliate_hunt__, 'affiliate'); // huntType says 'vip' — category must not
  // A PERSONAL hunt carrying the legacy per-user huntType 'vip' is publicly just a community hunt:
  // HUNT_CATEGORIES is only ['community','solo'], so 'vip'/'affiliate' are reachable ONLY by
  // shared key. That is what keeps someone's old personal VIP hunt out of the hub's featured row.
  assert.strictEqual(by.u1, 'community');
});
