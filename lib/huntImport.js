...
const { modHuntKey, affiliateHuntKey, vipHuntKey, CURRENCIES, uid } = require('./hunts-core');
...
// The five PUBLIC categories (see huntCategoryOf in hunts-core). THREE of them — `vip`,
// `affiliate` and `streamer` — are the tenant's shared hunts, identified by their KEY rather than
// by a storable huntType, with the behaviour type underneath. Storing "affiliate" as a huntType
// would produce a hunt no existing filter matches.
//
// `vip` belongs in that group and was missing from it: it mapped to a plain huntType with no key,
// so importing a VIP hunt produced a regular hunt owned by the tenant host that merely carried
// huntType 'vip' — and since huntCategoryOf no longer treats that as a VIP hunt, it round-tripped
// back as `community`. It was the one public category the API could not actually create.
const CATEGORY_TO_INTERNAL = {
  community: 'community', solo: 'solo',
  vip: 'vip',          // the VIP Hunt is VIP-shaped
...
function validateImport(body, { now, maxAgeMs = MAX_AGE_MS } = {}) {
  ...
  // Age gate. Track A passes the default (48h) so a leaked key can't inject arbitrary history;
  // the admin backfill passes Infinity because old history is exactly its job. Either way a FUTURE
  // hunt is already rejected above by `endedAt > now`.
...
function buildImportedHunt(value, { tenantId, hostDiscordId }) {
  const shared = value.huntType === 'affiliate' ? affiliateHuntKey(tenantId)
               ...
               : value.huntType === 'vip'       ? vipHuntKey(tenantId)
               : null;
