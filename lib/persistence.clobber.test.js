...
// A private directory, NOT the repo root. `node --test` runs test FILES in parallel, and this suite
// writes hunts_data.json / hunts_archive.json on nearly every case — sharing the repo root with the
// other persistence suites made "file writes are atomic" fail intermittently depending on
// interleaving. initPersistence takes dataDir precisely so each suite can own one.
const ROOT = fs.mkdtempSync(path.join(require('os').tmpdir(), 'ch-clobber-'));
...
test('a failed boot read BLOCKS Postgres writes instead of clobbering the row', async () => {
  ...
  await P.initPersistence({ pgPool: pool, dataDir: ROOT, normalizeSlot: (s) => String(s || '').toLowerCase() });
  ...
  // persistHunts only SCHEDULES a write — force it through, or this asserts nothing.
  await P.flushHunts();
...
test('a successful boot read on an EMPTY database still allows writes', async () => {
  ...
  // One row per hunt: params[0] = user ids, params[1] = the serialized hunts.
  assert.deepStrictEqual(pool.writes[0].params[0], ['someone']);
  assert.match(pool.writes[0].params[1][0], /"huntId":"h1"/);
});
...
test('file writes are atomic: valid JSON on disk and no leftover temp files', async () => {
  const P = freshModule();
  await P.initPersistence({ pgPool: null, dataDir: ROOT, normalizeSlot: (s) => String(s || '').toLowerCase() });
...
test('a healthy write reports pgLastWriteOk true', async () => {
  ...
  P.persistHunts();
  await P.flushHunts();                            // schedules, then settles the write promise
...
test('no write yet is null, NOT a failure', async () => {
  const P = freshModule();
  await P.initPersistence({ pgPool: fakePool(), dataDir: ROOT, normalizeSlot: (s) => String(s || '').toLowerCase() });
...
test('a write DURING the boot window (before the read completes) must not reach Postgres', async () => {
  ...
  const booting = P.initPersistence({ pgPool: pool, dataDir: ROOT, normalizeSlot: (s) => String(s || '').toLowerCase() });
  ...
  P.persistHunts();
  P.persistArchive();
  await P.flushHunts();   // force the scheduled write, so "no writes" is a real result
  ...
  // A flush only writes rows whose CONTENT changed, so there has to be something to write —
  // an empty diff is now correctly a no-op rather than a pointless rewrite of the whole blob.
  P.hunts['someone'] = { huntId: 'h1', bonuses: [], equity: [], calls: [] };
  P.persistHunts();
  await P.flushHunts();
