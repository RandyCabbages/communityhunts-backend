...
const { isCategory, keyForCategory, sharedRunHasWork, encodeRunKey, decodeRunKey,
        ...
        cleanSlots, findEquityRow } = require('./discordHunts');
...
describe('cleanSlots', () => {
  it('trims, collapses whitespace and drops blanks, keeping order', () => {
    assert.deepEqual(
      cleanSlots(['  Gates of  Olympus ', '', '   ', 'Sweet Bonanza']),
      ['Gates of Olympus', 'Sweet Bonanza'],
    );
  });

  it('stops at five, because that is the modal ceiling', () => {
    assert.equal(cleanSlots(['a', 'b', 'c', 'd', 'e', 'f', 'g']).length, 5);
  });

  it('caps a single name at 60 characters', () => {
    assert.equal(cleanSlots(['x'.repeat(200)])[0].length, 60);
  });

  it('keeps a repeat rather than collapsing it — the duplicate check answers that', () => {
    assert.deepEqual(cleanSlots(['CULT', 'CULT.']), ['CULT', 'CULT.']);
  });

  it('non-arrays and non-strings are nothing', () => {
    assert.deepEqual(cleanSlots(undefined), []);
    assert.deepEqual(cleanSlots('Gates'), []);
    assert.deepEqual(cleanSlots([1, null, {}, 'Real']), ['Real']);
  });
});

describe('findEquityRow', () => {
  const BEAN = '135203806676779008';
  const CABBAGE = '197365493516992512';
  const STRANGER = '999000999000999000';
  ...
  it('matches on discordId first', () => {
    const row = findEquityRow(equity, { discordId: CABBAGE, name: 'someone else' });
    assert.equal(row.id, 'r1');
  });

  it('falls back to the name, so an unlinked winner is still recognised', () => {
    assert.equal(findEquityRow(equity, { discordId: null, name: 'sverrir' }).id, 'r2');
  });
  ...
  it('an id nobody on the sheet holds falls through to the name', () => {
    assert.equal(findEquityRow(equity, { discordId: STRANGER, name: 'Cabbage' }).id, 'r1');
  });

  it('returns null for someone who is not on the sheet at all', () => {
    assert.equal(findEquityRow(equity, { discordId: STRANGER, name: 'Stranger' }), null);
    assert.equal(findEquityRow(equity, { discordId: null, name: '' }), null);
    assert.equal(findEquityRow(undefined, { discordId: null, name: 'Cabbage' }), null);
  });
});
