...
// The shared hunts archive under a fixed host key instead of a real user id.
// A host_user_id equality check alone would make them permanently unresolvable.
// '__mod_hunt__' is the pre-rebrand key for the tenant hunt (now '__tenant_hunt__') — both are
// kept here so payouts on ALREADY-ARCHIVED tenant hunts (immutable hunt_history rows still keyed
// under the old string) stay resolvable alongside newly-archived ones.
const MOD_HUNT_KEYS = new Set(['__tenant_hunt__', '__mod_hunt__', '__affiliate_hunt__', '__vip_hunt__']);
