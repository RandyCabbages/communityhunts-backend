...
// Caught by the first dry run: these rows collide only because their `name` fields
// are identical. The games are different, so neither may be dropped.
test('never drops a row whose game slug genuinely differs, only the studio prefix', () => {
  const liveFD = new Map([['floatingdragon', [{ url: 'pragmatic-play-floating-dragon-holdspin' }]]]);
  const entries = [
    { rainbetSlug: 'pragmatic-play-floating-dragon-holdspin', name: 'Floating Dragon', thumb: 'a' },
    { rainbetSlug: 'pragmatic-play-floating-dragon', name: 'Floating Dragon', thumb: 'b' },
  ];
  const r = planDedupe(entries, liveFD);
  assert.strictEqual(r.keep.length, 2);
  assert.strictEqual(r.drop.length, 0);
});
