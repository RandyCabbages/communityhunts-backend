...
function summarize(action, ctx = {}) {
  ...
  // Equity summaries read against the hunt's own label ("Bean's Hunt"), not a possessive —
  // these fire on shared mod/affiliate hunts too, where `targetName` IS the hunt name.
  if (action === 'equity.add')
    return `${who} added ${nameList(ctx.members)} to ${ctx.targetName || 'a hunt'}`;
  if (action === 'equity.remove')
    return `${who} removed ${nameList(ctx.members)} from ${ctx.targetName || 'a hunt'}`;
...
function recordHuntChange(req, before, after, o = {}) {
  ...
  if (d.removed.length || d.cleared) {
    ...
    recordFromReq(req, {
      ...
      summary: summarize(action, { actorName, targetName: o.targetName, removed: d.removed }),
      detail: {
        removed: d.removed,
  ...
  for (const [action, members] of [['equity.remove', m.removed], ['equity.add', m.added]]) {
    ...
    recordFromReq(req, {
      category: 'hunt', action, targetId: o.targetId || null,
      summary: summarize(action, { actorName, targetName: o.targetName, members }),
