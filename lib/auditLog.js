...
// ── Vault (base-game wins) ─────────────────────────────────────────
// Vault entries carry a stable id ({id, amount, note, ts}), so adds and removes key on it like
// equity. The deliberate DIFFERENCE from diffMembers: equity ignores amount edits, because a row's
// identity is who it is. For a vault entry the amount IS the event — it is money added to the
// hunt's winnings and nothing else about the row matters. A 2026-07-27 report of a vault figure
// being retyped repeatedly ("we had to tweak it a lot") left no trace anywhere precisely because
// nothing recorded that. Notes are ignored: only money movements earn a log line.
const vaultAmt = v => Number((v && v.amount) || 0) || 0;
function diffVault(before = [], after = []) {
  const b = Array.isArray(before) ? before : [];
  const a = Array.isArray(after) ? after : [];
  const byId = list => new Map(list.filter(v => v && v.id != null).map(v => [String(v.id), v]));
  const bMap = byId(b), aMap = byId(a);

  const added   = a.filter(v => v && v.id != null && !bMap.has(String(v.id)));
  const removed = b.filter(v => v && v.id != null && !aMap.has(String(v.id)));
  const changed = [];
  for (const [id, bv] of bMap) {
    const av = aMap.get(id);
    if (!av) continue;
    if (vaultAmt(bv) !== vaultAmt(av)) changed.push({ id, from: vaultAmt(bv), to: vaultAmt(av) });
  }
  return { added, removed, changed };
}

const money = n => Number(n || 0).toLocaleString('en-US');
const amountList = entries => (entries || []).map(v => money(vaultAmt(v))).join(', ');
...
function summarize(action, ctx = {}) {
  ...
  if (action === 'vault.add')
    return `${who} vaulted ${amountList(ctx.entries)} on ${where}`;
  if (action === 'vault.remove')
    return `${who} removed a ${amountList(ctx.entries)} vault entry from ${where}`;
  if (action === 'vault.change') {
    const moves = (ctx.changed || []).map(c => `${money(c.from)} → ${money(c.to)}`).join(', ');
    return `${who} changed a vault amount on ${where} (${moves})`;
  }
...
function recordHuntChange(req, before, after, o = {}) {
  ...
  // Vault — base-game money folded into the hunt's winnings. Adds/removes AND amount edits, the
  // last of which is the case equity deliberately ignores. See diffVault.
  const v = diffVault((before && before.vault) || [], (after && after.vault) || []);
  for (const [action, entries] of [['vault.add', v.added], ['vault.remove', v.removed]]) {
    if (!entries.length) continue;
    recordFromReq(req, {
      category: 'hunt', action, targetId: o.targetId || null,
      summary: summarize(action, { actorName, ...label, entries }),
      detail: { entries, before: snapshot() },
    });
  }
  if (v.changed.length) {
    recordFromReq(req, {
      category: 'hunt', action: 'vault.change', targetId: o.targetId || null,
      summary: summarize('vault.change', { actorName, ...label, changed: v.changed }),
      detail: { changed: v.changed, before: snapshot() },
    });
  }
}
...
module.exports = {
  keyOf, diffBonuses, diffMembers, diffVault, summarize,
