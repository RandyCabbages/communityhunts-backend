...
test('the flush NEVER stamps `ts` on an untimestamped call', async () => {
  // It briefly did. `calls[].at` was null on nearly every row of every hunt, and a flush-time
  // sweep looked like the one choke point no write path could miss. It is the wrong place: the
  // flush cannot tell a call that just arrived from one that has been unstamped since before the
  // field existed, so it stamped both — writing today's time onto historical calls and inventing
  // an order that never happened. stampNewCalls() (lib/hunts-core.js) owns this at the six write
  // sites, where `prev` exists and "is this row new?" is actually answerable.
  const P = freshModule();
  const pool = fakePool();
  await boot(P, pool);
  ...
  assert.ok(!('ts' in written[0]), 'an unstamped call must reach the row still unstamped');
  assert.strictEqual(written[1].ts, 1234, 'and an existing timestamp is never rewritten');
});

test('an unchanged hunt does not rewrite its row on the next flush', async () => {
  // The failure mode a flush-time stamp would have caused: a `ts` minted per flush makes the
  // content diff see a change forever, so every hunt rewrites its row every time — exactly the
  // write amplification the per-row split exists to remove.
  const P = freshModule();
  const pool = fakePool();
  await boot(P, pool);
