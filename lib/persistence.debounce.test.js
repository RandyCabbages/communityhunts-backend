...
test('the coalesced write carries the LAST state, not the first', async () => {
  ...
  const written = JSON.parse(pool.writes[0].params[0]);
...
test('a later persistHunts() after a flush schedules a NEW write', async () => {
  ...
  assert.strictEqual(JSON.parse(pool.writes[1].params[0]).a.huntId, 'h2');
});
...
test('call arrays are still deduped before the blob is written', async () => {
  const P = freshModule();
  const pool = fakePool();
  await boot(P, pool);
...
test('flushAll() also waits for an in-flight archive write', async () => {
  ...
  const archiveWrites = pool.writes.filter(w => /VALUES\('archive'/.test(w.sql));
