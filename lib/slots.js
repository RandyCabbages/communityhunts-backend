...
const slotStore = require('./rainbetSlotStore');
...
// a wrong slug 404s). Mirrors scripts/check_new_slots.js.
const SLUG_FIXES = {
  'hacksaw-chaos-crew-2': 'hacksaw-chaos-crew-ii',
  // Rainbet serves this as one word, hacksaw-phoenix-duelreels (the hyphenated form
  // 404s). Remapping here collapses slot.report's "Phoenix Duel Reels" onto the real
  // rainbet_slots.json row in the search pool instead of adding a broken duplicate.
  ...
  // pragmatic-play-big-bass-keeping-it-reel (the bonanza form 404s). Collapses onto
  // the real row instead of re-adding the broken "Big Bass Bonanza: Keeping it Reel".
  'pragmatic-play-big-bass-bonanza-keeping-it-reel': 'pragmatic-play-big-bass-keeping-it-reel',
  // slot.report's "mr-null-s-wicked-wares" slug has no real thumb (slot.report placeholder);
  // collapse onto the reviewed "mr-nulls-wicked-wares" row (real Rainbet-CDN thumb) instead of
  // surfacing the broken duplicate in the search pool. Verified on rainbet.com.
  'pragmatic-play-mr-null-s-wicked-wares': 'pragmatic-play-mr-nulls-wicked-wares',
  // slot.report's "six-six-six" row only carried a slot.report thumb; collapse onto the reviewed
  // "sixsixsix" row (real Rainbet-CDN thumb SixSixSix.png) instead of surfacing the broken
  // duplicate in the search pool. Rainbet serves this Hacksaw game as one word. Verified on rainbet.com.
  'hacksaw-six-six-six': 'hacksaw-sixsixsix',
  // slot.report renders an apostrophe as its own "-s-" segment; Rainbet keeps the clean
  // form. Collapses the duplicate onto the real rainbet_slots.json row instead of
  // surfacing a second, dead-thumbed entry in the search pool. Verified via the games API.
...
// The committed snapshot. Since the catalogue moved into Postgres (lib/rainbetSlotStore.js) this
// is the first-boot seed and the local-dev source, NOT what keeps the catalogue alive — see the
// header of that module for why a file could not do that job.
function readRainbetSlotsFile() {
  try {
    if (!fs.existsSync(RAINBET_SLOTS_FILE)) {
      console.log('[slots] rainbet_slots.json not found, using slot.report only');
      return [];
    }
    return JSON.parse(fs.readFileSync(RAINBET_SLOTS_FILE, 'utf8'));
  } catch (e) {
    console.error('[slots] Failed to read rainbet_slots.json:', e.message);
    return [];
  }
}

// Shared by the file and Postgres paths — both hand over the same { name, rainbetSlug, thumb }
// shape, so the normalization has exactly one home.
function mapRainbetEntries(raw) {
  try {
    ...
    console.error('[slots] Failed to map the Rainbet catalogue:', e.message);
    return [];
  }
}

function loadRainbetSlots() {
  const mapped = mapRainbetEntries(readRainbetSlotsFile());
  console.log(`[slots] Loaded ${mapped.length} slots from rainbet_slots.json`);
  return mapped;
}

// Boot seed is the FILE, synchronously, exactly as before: the search pool has to be usable the
// instant the process serves traffic, and the Postgres read is async. adoptRainbetSlotStore()
// swaps in the durable catalogue a moment later. If Postgres is unreachable we simply stay on the
// file, which is the behaviour this module had all along.
let RAINBET_SLOTS = loadRainbetSlots();

// Swap the in-memory catalogue for a set of raw { name, rainbetSlug, thumb } entries. Used by the
// sync, which already holds the merged list it just computed — no reason to make it round-trip
// through disk or Postgres to find out what it wrote.
function setRainbetSlots(raw) {
  RAINBET_SLOTS = mapRainbetEntries(raw);
  LIVE_NAMES = loadLiveNames();
  rebuildSearchPool();
  return RAINBET_SLOTS.length;
}

// Re-reads rainbet_slots.json from disk and refreshes the in-memory search pool.
// Kept for the file-only path (local dev, and any caller that has written the file
// rather than handing over entries).
...
// Move the catalogue onto Postgres. Seeds the table from the committed file the first time (and
// only when the table is empty — it can never clobber a live catalogue with a stale snapshot),
// then serves from the database.
//
// Deliberately best-effort: a failure here leaves the file-loaded pool in place, so the worst case
// is the behaviour that existed before this module did.
async function adoptRainbetSlotStore({ pgPool } = {}) {
  if (!pgPool) return { adopted: false, reason: 'no-db' };
  try {
    await slotStore.initRainbetSlotStore({ pgPool });
    await slotStore.seedIfEmpty(readRainbetSlotsFile());
    const rows = await slotStore.loadAll();
    if (!rows || rows.length === 0) return { adopted: false, reason: 'empty' };
    const n = setRainbetSlots(rows);
    console.log(`[slots] catalogue now served from Postgres (${n} slots)`);
    return { adopted: true, count: n };
  } catch (e) {
    console.error('[slots] could not adopt the Postgres catalogue, staying on the file:', e.message);
    return { adopted: false, reason: e.message };
  }
}
...
const imgProxyCache = new Map(); // url -> {buf, ct, at} — insertion-ordered, so FIFO eviction is .keys().next()
const IMG_PROXY_TTL = 24 * 60 * 60 * 1000; // 24 hours
// The cache MUST be bounded. This route is unauthenticated and behind no rate limiter, and the
// key is the full URL — the host allowlist says nothing about the query string, so
// `…/x.png?cb=1`, `?cb=2`, … are unlimited distinct keys each holding an image buffer. Without a
// cap that is a remote memory-exhaustion path (the TTL alone never freed anything: it only
// decided whether an entry was SERVED). 512 × ~250 KB ≈ 128 MB worst case.
const IMG_PROXY_MAX_ENTRIES = 512;
const IMG_PROXY_MAX_BYTES = 8 * 1024 * 1024; // don't let one oversized asset dominate the cache

// Drop expired entries, then oldest-first until we're under the cap.
function trimImgProxyCache() {
  const now = Date.now();
  for (const [k, v] of imgProxyCache) {
    if (now - v.at >= IMG_PROXY_TTL) imgProxyCache.delete(k);
  }
  while (imgProxyCache.size >= IMG_PROXY_MAX_ENTRIES) {
    const oldest = imgProxyCache.keys().next();
    if (oldest.done) break;
    imgProxyCache.delete(oldest.value);
  }
}
...
async function imgProxyHandler(req, res) {
  ...
  try {
    ...
    // Check cache — an expired entry is deleted, not merely ignored.
    const cached = imgProxyCache.get(url);
    if (cached && Date.now() - cached.at >= IMG_PROXY_TTL) imgProxyCache.delete(url);
    ...
    // Re-check the allowlist against where the redirect chain actually LANDED. fetch follows up
    // to 20 redirects, and the host check above only ever saw the URL the caller supplied — so an
    // allowlisted host with an open redirect (or a compromised CDN) could otherwise point this at
    // anything the container can reach, including link-local metadata endpoints.
    if (resp.url) {
      try {
        if (!ALLOWED_IMG_HOSTS.includes(new URL(resp.url).hostname)) {
          return res.status(403).json({ error: 'Redirected to a host that is not allowed' });
        }
      } catch { return res.status(400).json({ error: 'Bad redirect target' }); }
    }
    if (!resp.ok) return res.status(resp.status).end();
    const ct = resp.headers.get('content-type') || 'image/jpeg';
    if (!ct.startsWith('image/')) return res.status(400).json({ error: 'Not an image' });
    const buf = Buffer.from(await resp.arrayBuffer());
    // Still served, just not retained — one huge asset must not evict the whole working set.
    if (buf.length <= IMG_PROXY_MAX_BYTES) {
      trimImgProxyCache();
      imgProxyCache.delete(url);   // re-insert so the key moves to the end of the eviction order
...
function rebuildSearchPool() {
  ...
  for (const s of RAINBET_SLOTS) {
    // Keyed by canonKey, not the literal slug: slot.report ships the same game under
    // provider-alias, apostrophe and word-join variants, and a literal-slug seen-set
    // lets every one of them through as a duplicate row.
    const key = (s.rainbetSlug || '').toLowerCase();
    if (key) seenSlugs.add(canonKey(key));
    seenNames.add(normNameKey(s.name));
    pool.push({
  ...
  if (slotCache.games.length > 0) {
    ...
    for (const g of slotCache.games) {
      // slot.report suffixes some Pragmatic entries "… Slot by Pragmatic Play";
      // Rainbet's real slug/name never carries it (CULT. lives at pragmatic-play-cult),
      // so strip it or the constructed slug 404s and the row shadows the real one in
      // search (mirrors scripts/check_new_slots.js).
      const name = g.name.replace(/\s+slot by pragmatic play.*$/i, '');
      const slug = g.slug.replace(/-slot-by-pragmatic-play.*$/, '');
      if (seenNames.has(normNameKey(name))) continue;
      ...
      // Ghost guard (2026-07-22): only merge a slot.report slot that has a TRUSTED,
      // known-live thumbnail — slot.report-reviewed (thumbMap), a pre-verified SoftSwiss
      // hit (SOFTSWISS_HITS), or a hardcoded extra. The old last-resort SoftSwiss *guess*
      // (cdn.softswiss.net/i/s4/{provider}/{Pascal}.webp) produced dead links for the many
      // games Rainbet doesn't actually carry, so the picker filled with missing-thumb
      // "ghost" rows. Verified new slots reach the pool via the file (step 1) once
      // scripts/check_new_slots.js writes them (it GET-verifies thumbs before adding), so
      // dropping the guess here loses no real coverage — it only stops the ghost rows.
      if (!thumb) continue;

      pool.push({
        name, slug, provider: g.provider_slug,
...
async function slotsSearchHandler(req, res) {
  ...
  // Match on a space/punctuation-stripped key so a call typed one way finds the
  // catalog entry spelled another way: "Phoenix Duel Reels" → "Phoenix DuelReels",
  // "mr null" → "Mr. Null's", "Cosmic Clusters" → "Cosmic Clusters!". The old raw
  // `name.includes(q)` matcher missed every spacing/punctuation mismatch, leaving the
  // called slot with no thumb (blank SVG tile). Stripping is a superset of the old
  // matcher, so it never drops a result that used to match.
  const searchKey = s => s.toLowerCase().replace(/&/g, 'and').replace(/[^a-z0-9]/g, '');
  const nq = searchKey(q);

  const pool = _searchPool;
  const filtered = (q.length >= 2 && nq)
    ? pool.filter(g => searchKey(g.name).includes(nq))
    : pool;
  ...
  const results = filtered
    ...
    .sort((a, b) => {
      if (q.length >= 2 && nq) {
        const aStarts = searchKey(a.name).startsWith(nq);
        const bStarts = searchKey(b.name).startsWith(nq);
...
module.exports = {
  ...
  reloadRainbetSlots,
  setRainbetSlots,
  adoptRainbetSlotStore,
  readRainbetSlotsFile,
  loadLiveNames,
  thumbForSlotNames,
};
