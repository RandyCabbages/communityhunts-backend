...
// Annual price IDs — hardcoded (static, created via MCP 2026-07-06).
const ANNUAL_PRICE_MAP = {
  basic:    () => 'price_1Tq6MbRrpLw0LHPUDi7oxfRQ',
  pro:      () => 'price_1Tq6McRrpLw0LHPUynTDL7KU',
  ultimate: () => 'price_1Tq6MdRrpLw0LHPUJVHISZrR',
};
// Community plan price IDs — created live via MCP 2026-07-08 (self-serve community onboarding).
// Env override for flexibility; hardcoded fallback so it works without extra Railway config.
const COMMUNITY_PRICE_MAP = {
  community_starter: () => process.env.STRIPE_PRICE_COMMUNITY_STARTER || 'price_1Tqp7iRrpLw0LHPUQw6g9xt4',
  community_pro:     () => process.env.STRIPE_PRICE_COMMUNITY_PRO     || 'price_1Tqp7qRrpLw0LHPUuXucZ4k9',
  // Partner tier — create the Stripe price and set STRIPE_PRICE_COMMUNITY_PARTNER in Railway.
  // No hardcoded fallback: createCommunityCheckoutSession throws "Price not configured" until
  // the env var is set, rather than checking out against a bogus id.
  community_partner: () => process.env.STRIPE_PRICE_COMMUNITY_PARTNER || '',
};
const COMMUNITY_PLANS = new Set(['community_starter', 'community_pro', 'community_partner']);
