...
function buildPriceTierMap() {
  ...
  for (const [tier, priceFn] of Object.entries(COMMUNITY_PRICE_MAP)) {
    const priceId = priceFn();
    if (priceId) TIER_FROM_PRICE[priceId] = tier;
  }
...
async function createExtensionSubscriptionSession(userId, successUrl, cancelUrl, email, displayName, interval) {
  if (!stripe) throw new Error('Stripe not configured');
  const annual = interval === 'year';
  const priceId = annual
    ? (process.env.STRIPE_PRICE_EXT_FULL_ANNUAL || process.env.STRIPE_PRICE_EXT_FULL)
    : process.env.STRIPE_PRICE_EXT_FULL;
