...
function getHuntStats(tenantId, tz) {
  ...
  let completedHunts = 0, potSum = 0, potHunts = 0;
  ...
  for (const h of list) {
    ...
    if (h.user?.id) {
      const e = hunterMap.get(h.user.id) || { userId: h.user.id, name: '', avatar: null, hunts: 0, totalWon: 0 };
      e.hunts++;
      ...
      e.totalWon += (h.bonuses||[]).reduce((s,b)=>s+(+b.win||0),0);
