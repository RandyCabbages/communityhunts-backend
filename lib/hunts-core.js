...
//
// ORDER MATTERS: an affiliate hunt matches both the shared-key branch and the huntType branch,
// so the shared-key checks must come first.
const HUNT_CATEGORIES = ['vip', 'community', 'solo'];
...
function getPublicHunts(tenantId)   { return Object.values(hunts).filter(h=>h.isLive && huntHasContent(h) && inTenant(h,tenantId) && h.user?.id !== modHuntKey(tenantId) && h.user?.id !== affiliateHuntKey(tenantId) && h.user?.id !== vipHuntKey(tenantId)).map(huntSummary); }
...
module.exports = {
  ...
  emitHubUpdate, publicHuntView, emitHuntUpdate, emitToHuntRoom, huntHasAnon,
  bindEquityIdentityByName, linkEquityMember, initialEquity, preserveRowIdentity,
