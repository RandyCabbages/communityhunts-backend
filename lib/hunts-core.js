...
function getHuntStats(tenantId, tz) {
  ...
  return {
    summary: {
      totalHunts: all.length, completedHunts,
      liveNow: current.filter(h=>h.isLive && !h.archivedAt).length,
      totalBonuses, openedBonuses, totalBet, totalWon,
      overallMulti: openedBet > 0 ? totalWon/openedBet : null,
      avgSlotsPerHunt: all.length ? totalBonuses/all.length : 0,
      avgPot: potHunts ? potSum/potHunts : 0,
      types,
    },
    topGotIn, topCalled, topHunters, biggestHits,
    hours, weekdays, weeks, tz: zone,
  };
}
...
module.exports = {
  ...
  getPublicHunts, getArchivedHunts, getAllHunts, getSlotCallCounts, getGotInLog, getHuntStats,
