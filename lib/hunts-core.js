...
function runnerNameOf(h) {
  ...
  return shouldMaskIdentity({ discordId: row.discordId, name }) ? null : name;
}

function huntSummary(h) {
  return {
    userId: h.user?.id, username: runnerNameOf(h) || h.user?.displayName, avatar: h.user?.avatar,
