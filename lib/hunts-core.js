...
// Hub "live now" filter. A COMPLETED hunt (every bonus opened) is done, so it's excluded here
// even if it's still isLive — this hides a finished hunt from the directory instantly, without
// waiting on the frontend's best-effort /end call or the janitor's completed-reap sweep. The
// janitor still archives it (→ Archived tab) within ~10m for the failure/closed-tab case.
function getPublicHunts(tenantId)   { return Object.values(hunts).filter(h=>h.isLive && huntHasContent(h) && !huntCompleted(h) && inTenant(h,tenantId) && h.user?.id !== modHuntKey(tenantId) && h.user?.id !== affiliateHuntKey(tenantId)).map(huntSummary); }
...
function getHuntStats(tenantId, tz) {
  ...
  // One currency slice: the full aggregation over just that group's hunts.
  const statsFor = (list) => {
  ...
  let totalBonuses = 0, openedBonuses = 0, totalBet = 0, openedBet = 0, totalWon = 0;
  ...
  for (const h of list) {
    ...
    const huntWon = (h.bonuses||[]).reduce((s,b)=>s+(+b.win||0),0);
    ...
    if (pot > 0) { potSum += pot; potHunts++; potWon += huntWon; }
    ...
    if (h.user?.id) {
      ...
      if (pot > 0) { e.totalPot += pot; e.potWon += huntWon; e.pnlHunts++; }
    ...
    for (const b of h.bonuses) {
      ...
      if (win > 0) {
        openedBonuses++; openedBet += bet; totalWon += win;
  ...
  return {
    summary: {
      ...
      totalBonuses, openedBonuses, totalBet, totalWon,
      overallMulti: openedBet > 0 ? totalWon/openedBet : null,
  ...
  const byCurrency = {};
  for (const code of currencies) byCurrency[code] = statsFor(groups.get(code));
  return {
...
function huntHasAnon(h) {
  return Array.isArray(h.equity) && h.equity.some(e => e.discordId && isAnonymousUser(e.discordId));
}
...
function maskEquityMember(member, viewerId, privileged) {
  const { discordId, ...e } = member;
  if (!discordId || !isAnonymousUser(discordId)) return e;
  const isSelf = viewerId && String(viewerId) === String(discordId);
...
function publicHuntView(h, viewerId) {
  ...
  return {
    ...
    equity: Array.isArray(h.equity)
      ? h.equity.map(e => maskEquityMember(e, viewerId, privileged))
      : h.equity,
  };
}
...
module.exports = {
  ...
  getPublicHunts, getArchivedHunts, getAllHunts, getSlotCallCounts, getGotInLog, getHuntsFullExport, getHuntStats,
