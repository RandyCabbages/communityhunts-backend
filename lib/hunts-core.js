...
const { badgedIds } = require('./callPriority'); // roster -> Set of every badge-holding id
...
// Hunt -> the tenant's badge roster ({owners, king, mods, supporters}). Injected because the
// roster lives in lib/tenants + lib/supporters, which hunts-core must not depend on. Default
// yields no badges, so an un-wired build simply ships an unprioritized queue.
let badgeRosterFor = () => null;
...
function initHuntsCore(deps) {
  ...
  if (deps.badgeRosterFor)     badgeRosterFor     = deps.badgeRosterFor;
}
...
// Public call rows, carrying the two ordering hints that `callerId` would otherwise have supplied.
//
// The call queue is a round-robin BY CALLER, and every public surface renders it: the share page,
// a non-owner watching a live hunt, the OBS overlay. All of them lose `callerId` to
// maskCallerEntry, so without help they answer "who is one caller?" with the display NAME — and
// the four writers that create calls disagree on that string (the backend stamps
// `user.displayName` for share-link and Discord-bot calls; Pull List / Generate / Preferred Slots
// write the EQUITY ROW name). One person then occupies two slots in the rotation, takes two turns
// a round, and out-calls everyone. Same reason `priority` exists: the host reads the badge off
// `badgeFor(call.callerId)` and a public viewer has no id to look up.
//
// Both are attached HERE rather than zipped on at each route because emitHuntUpdate re-serializes
// through this function on every hunt change (~every 500ms in a live hunt). A route-only fix would
// be undone by the next socket frame, and the viewer's queue would reshuffle continuously.
//
// `callerKey` is a per-RESPONSE sequence number, not a hash of the id — deliberately weaker than
// lib/publicIds.js, because grouping only has to hold WITHIN one payload. Nothing durable or
// correlatable crosses the wire and there is no digest to test a candidate id against.
const normCaller = s => String(s || '').trim().toLowerCase().replace(/\s+/g, '');
function publicCallRows(h, viewerId, privileged) {
  let badged = new Set();
  // Never let a roster lookup break the hunt payload — ordering is cosmetic, the hunt is not.
  try { badged = badgedIds(badgeRosterFor(h) || {}); }
  catch (e) { console.error('[hunt] badge roster lookup failed -', e && e.message); }
  const keys = new Map();
  return h.calls.map(c => {
    const masked = maskCallerEntry(c, 'user', viewerId, privileged);
    const id = c && c.callerId;
    // Mirrors the frontend's callerKey() preference: verified id first, normalized name for rows
    // that were never linked to a Discord account.
    const identity = id ? `id:${String(id)}` : `name:${normCaller(c && c.user)}`;
    if (!keys.has(identity)) keys.set(identity, `c${keys.size}`);
    const row = { ...masked, callerKey: keys.get(identity) };
    // Only `true` is ever written, so unbadged rows stay shaped as they are and clients read
    // `!!c.priority`.
    if (id && badged.has(String(id))) row.priority = true;
    return row;
  });
}
...
function publicHuntView(h, viewerId) {
  ...
  return {
    ...
    calls: Array.isArray(h.calls) ? publicCallRows(h, viewerId, privileged) : h.calls,
