...
function flushHunts() {
  ...
  //
  // The same pass stamps `ts` on any call that lacks one. lib/huntCalls.js already stamps the
  // calls IT creates (the public board and the Discord bot), but the host's own calls are minted
  // client-side and arrive as a whole array on the next PUT, so the overwhelming majority carried
  // no timestamp at all — the public API's `calls[].at` was null on every row of every hunt, and a
  // consumer had no way to order or time the queue. Stamped HERE rather than at the six write
  // sites that assign `h.calls`, because a scheme requiring each writer to remember fails silently
  // the first time one is added (the same argument as persistHunts' content-diffing).
  //
  // The value therefore means "first persisted by the server", not "the instant someone typed it".
  // Within 250ms of the call landing, which is all any consumer can use it for. Existing rows are
  // stamped on the first flush after deploy; the archive is left alone, so history is not rewritten.
  ...
  for (const id in hunts) {
    ...
    if (h?.calls?.length) {
      ...
      h.calls = h.calls.filter(c => {
        ...
        seen.add(key);
        return true;
      }).map(c => (typeof c.ts === 'number' ? c : { ...c, ts: nowTs }));
    }
  }
