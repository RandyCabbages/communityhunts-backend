...
// Max hunts kept in the in-memory archive (newest first). The durable, uncapped
// source of truth is the Postgres hunt_history table (via statsStore); this cap only
// bounds the in-memory list the Hub's Archived tab renders.
...
function archiveHunt(hunt) {
  ...
  if (idx !== -1) {
    ...
    if (archive.length > ARCHIVE_CAP) archive.splice(ARCHIVE_CAP);
  }
  persistArchive();
