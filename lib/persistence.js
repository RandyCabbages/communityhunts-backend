...
// ── One row per hunt ──────────────────────────────────────────────────────────
// The debounce above capped the write RATE, but every flush still rewrote the entire hunt map as
// one JSONB value: change 50 bytes on one hunt, rewrite all of them. Three costs, all growing with
// the number of concurrent hunts — the PG write itself, the WAL that write generates (which is now
// also the PITR archive), and the serialization that blocks the single event loop serving every
// tenant.
//
// hunts_rows stores one row per hunt, and a flush writes only the rows whose CONTENT changed.
//
// Changed-ness is decided by comparing serialized state against what was last written, NOT by
// tracking dirty ids. persistHunts() has 52 call sites and takes no arguments, so any scheme that
// asks callers to name what they touched fails silently the first time a site is missed — and the
// failure is "this hunt stopped being saved", which is the worst bug this file can have. Comparing
// content cannot miss a change.
//
// Free consequence worth knowing: a failed write leaves lastWrittenHunts un-updated, so the next
// flush naturally retries exactly the rows that didn't land. The old whole-blob write simply lost
// them until something else changed.
const lastWrittenHunts = new Map(); // userId -> the JSON string last durably written
let huntsMigratedFromBlob = false;  // seeded from hunts_kv this boot; first flush writes every row

// ── The archive gets the same treatment ───────────────────────────────────────
// It was in worse shape than hunts: one blob rewritten in full on every hunt end, growing forever
// (281 entries in production at the time of writing), and — unlike hunts — never debounced at all.
// 21 call sites, several of which fire in bursts (retryPendingStats sweeps, the janitor's reap).
//
// Deliberately still not debounced. Per-row diffing already removes the amplification, and an
// unchanged persistArchive() now writes nothing, which is the same win. Adding a timer would mean
// flushAll() has to drain it too, and a missed drain on SIGTERM loses an archived hunt.
const lastWrittenArchive = new Map(); // archiveId -> the JSON string last durably written
let archiveMigratedFromBlob = false;

// A stable primary key for an archived snapshot. sameHuntInstance() is the in-memory notion of
// identity, but it falls back to (user.id, startedAt) when huntId is absent, and neither is
// guaranteed present on older entries — a null or colliding PK silently merges two hunts into one
// row. So each snapshot carries its own id, generated once and persisted with it.
//
// Assigning here MUTATES the entry, which makes it differ from the baseline and therefore get
// written. That is the intent: backfilling is a real change to the stored value.
function archiveKey(h) {
  if (!h.archiveId) h.archiveId = crypto.randomBytes(10).toString('hex');
  return h.archiveId;
}
...
async function initPersistence(deps) {
  ...
  if (pgPool) {
    ...
    `).then(() => pgPool.query(`
      CREATE TABLE IF NOT EXISTS hunts_rows (
    ...
    `)).then(() => pgPool.query(`
      ...
      CREATE INDEX IF NOT EXISTS archive_rows_archived_at_idx ON archive_rows (archived_at DESC);
    `))
      .then(() => console.log('[persist] Postgres hunts_kv + hunts_rows tables ready'))
      .catch(e => { console.error('[persist] hunts table init failed:', e.message); });
  }
...
async function loadPersistedState() {
  ...
  if (pgPool) {
    try {
      ...
      // Per-row store first. The hunts_kv blob is only read when hunts_rows is EMPTY, which is
      // either a genuinely fresh database or the one boot that migrates an existing blob across.
      const rowsRes = await pgPool.query('SELECT user_id, data FROM hunts_rows');
      if (rowsRes.rows.length) {
        for (const r of rowsRes.rows) {
          hunts[r.user_id] = r.data;
          // Seed the comparison baseline from what is actually durable, so the first flush writes
          // only what has changed since — not every row.
          lastWrittenHunts.set(r.user_id, JSON.stringify(r.data));
        }
        loadedFromPg = true;
      } else {
        ...
        if (huntsRow.rows[0]) {
          ...
          loadedFromPg = true;
          // Baseline deliberately left EMPTY: every hunt then reads as changed, so the first flush
          // copies the whole blob into hunts_rows. The blob itself is left alone — frozen as a
          // last-known-good that a code revert can still read.
          if (Object.keys(hunts).length) huntsMigratedFromBlob = true;
        }
      }
      // Order is not stored as a position: the array's canonical order IS archivedAt descending
      // (the boot dedupe below sorts by exactly that), so it is derived on read. Tracking indices
      // would mean every insert rewrote every row after it.
      const archRes = await pgPool.query(
        'SELECT data FROM archive_rows ORDER BY archived_at DESC NULLS LAST'
      );
      if (archRes.rows.length) {
        for (const r of archRes.rows) {
          archive.push(r.data);
          lastWrittenArchive.set(archiveKey(r.data), JSON.stringify(r.data));
        }
      } else {
        ...
        if (archiveRow.rows[0]) {
          ...
          if (archive.length) archiveMigratedFromBlob = true;
        }
      }
      ...
      if (huntsMigratedFromBlob) {
        console.log(`[persist] Migrating ${Object.keys(hunts).length} hunts from the hunts_kv blob into hunts_rows`);
        persistHunts(); // writes every row, since nothing is in the baseline yet
      }
      if (archiveMigratedFromBlob) {
        console.log(`[persist] Migrating ${archive.length} archived hunts from the hunts_kv blob into archive_rows`);
        persistArchive();
      }
...
function flushHunts() {
  ...
  //
  // This pass does NOT stamp `ts` on untimestamped calls, and must not. It briefly did: the public
  // API's `calls[].at` was null on nearly every row, and a flush-time stamp looked like the one
  // choke point no write path could miss. It is the wrong place, because the flush cannot tell a
  // call that just arrived from one that has sat unstamped since before the field existed — so it
  // stamped BOTH, writing today's timestamp onto historical calls and inventing an ordering that
  // never happened. `stampNewCalls()` (lib/hunts-core.js) replaced it at all six client-supplied-
  // `calls` write sites, where `prev` is available and "new row" is answerable; it leaves a
  // pre-existing unstamped row alone on purpose. Add it to any seventh write site rather than
  // reaching for a sweep here.
  for (const id in hunts) {
    const h = hunts[id];
    if (h?.calls?.length) {
      const seen = new Set();
  ...
  if (pgWritable('hunts')) return track(flushHuntRows());
...
// Write only the hunts whose serialized content differs from what was last durably written, and
// delete the rows for hunts that are gone. Never rejects.
//
// Deletion is the one thing the blob got for free: a hunt removed from the map simply wasn't in the
// next write. With rows it has to be explicit, and a missed DELETE means the hunt RESURRECTS on the
// next boot. The baseline is the source of truth for "what does the database currently hold", so
// anything in it that is no longer in `hunts` is a deletion.
async function flushHuntRows() {
  const ids = [];
  const blobs = [];
  for (const id in hunts) {
    const s = JSON.stringify(hunts[id]);
    if (lastWrittenHunts.get(id) !== s) { ids.push(id); blobs.push(s); }
  }
  const gone = [];
  for (const id of lastWrittenHunts.keys()) if (!(id in hunts)) gone.push(id);
  if (!ids.length && !gone.length) return;

  try {
    if (ids.length) {
      // One statement for the whole batch — a burst of edits across several hunts costs one round
      // trip, not one per hunt.
      await pgPool.query(
        `INSERT INTO hunts_rows(user_id, data)
    ...
    if (gone.length) {
      await pgPool.query('DELETE FROM hunts_rows WHERE user_id = ANY($1::text[])', [gone]);
      console.log(`[persist] Deleted ${gone.length} hunt row(s) from Postgres`);
    }
    // Only advance the baseline once the write actually landed. On failure it stays put, so the
    // next flush retries exactly these rows.
    for (let i = 0; i < ids.length; i++) lastWrittenHunts.set(ids[i], blobs[i]);
    for (const id of gone) lastWrittenHunts.delete(id);
    notePgWrite('hunts', null);
  } catch (e) {
    notePgWrite('hunts', e);
    console.error('[persist] PG save hunts failed:', e.message);
  }
}
...
function persistArchive() {
  if (pgWritable('archive')) track(flushArchiveRows());
  try { writeFileAtomic(ARCHIVE_FILE, JSON.stringify(archive)); }
  catch(e) { /* file write may fail on ephemeral disk */ }
}

// Write only the archived snapshots whose content changed, and delete the rows for ones that are
// gone. Same contract as flushHuntRows: never rejects, and the baseline advances only on success so
// a failed write retries itself on the next call.
//
// Deletions are real here, not theoretical — trimArchive evicts past the per-tenant cap,
// unarchiveHunt removes a reopened hunt, and both the admin delete and the janitor splice entries
// out. Every one of those used to disappear for free by simply not being in the next blob.
async function flushArchiveRows() {
  const keys = [], blobs = [], ats = [];
  const live = new Set();
  for (const h of archive) {
    if (!h) continue;
    const k = archiveKey(h);
    live.add(k);
    const s = JSON.stringify(h);
    if (lastWrittenArchive.get(k) !== s) { keys.push(k); blobs.push(s); ats.push(h.archivedAt || null); }
  }
  const gone = [];
  for (const k of lastWrittenArchive.keys()) if (!live.has(k)) gone.push(k);
  if (!keys.length && !gone.length) return;

  try {
    if (keys.length) {
      await pgPool.query(
        `INSERT INTO archive_rows(archive_id, data, archived_at)
         SELECT * FROM UNNEST($1::text[], $2::jsonb[], $3::text[])
         ON CONFLICT (archive_id) DO UPDATE SET data = EXCLUDED.data, archived_at = EXCLUDED.archived_at, updated_at = now()`,
        [keys, blobs, ats]
      );
    }
    if (gone.length) {
      await pgPool.query('DELETE FROM archive_rows WHERE archive_id = ANY($1::text[])', [gone]);
      console.log(`[persist] Deleted ${gone.length} archived hunt row(s) from Postgres`);
    }
    for (let i = 0; i < keys.length; i++) lastWrittenArchive.set(keys[i], blobs[i]);
    for (const k of gone) lastWrittenArchive.delete(k);
    notePgWrite('archive', null);
  } catch (e) {
    notePgWrite('archive', e);
    console.error('[persist] PG save archive failed:', e.message);
  }
}
...
function archiveHunt(hunt) {
  ...
  if (idx !== -1) {
    // Re-ending the same hunt REPLACES the snapshot with a fresh object. Carry the archive id over,
    // or the replacement gets a new one and the old row is orphaned in Postgres — a duplicate that
    // reappears on the next boot, which is exactly what the upsert here exists to prevent.
    snap.archiveId = archive[idx].archiveId;
    archive[idx] = snap;
  } else {
...
module.exports = { hunts, archive, shareTokens, initPersistence, persistHunts, persistArchive, persistShareTokens, tokenForOwner, archiveHunt, unarchiveHunt,
  // Exported so lib/huntReassign.js can resolve "the same hunt" exactly the way archiveHunt does.
  // A second copy of this rule would drift, and the failure mode is a hunt whose archived snapshot
  // and current record end up owned by different people.
  sameHuntInstance,
  flushHunts, flushAll,
  ...
  pgHealth: () => ({
    ...
    // Hunt rows the durable store is believed to hold. Diverging from the number of live hunts for
    // more than a flush window means writes or deletes are not landing.
    huntRowsWritten: lastWrittenHunts.size,
    archiveRowsWritten: lastWrittenArchive.size,
  }) };
