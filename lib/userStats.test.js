...
test('vault: a joined member reports their SHARE of it, not the whole hunt', () => {
  const hs = [{
    user: { id: 'host3' }, huntId: 'vs', currency: 'USD', usdRate: 1, archivedAt: '2026-07-07T00:00:00Z',
    // 100 of a 1000 pot = a 10% share.
    equity: [{ id: 'r-host', discordId: 'host3', amount: 900 },
             { id: 'r-me', discordId: 'member3', amount: 100 }],
    bonuses: [{ bet: 10, win: 400, mult: 40 }],
    vault: [{ amount: 600 }],
  }];
  // Won 400 + 600 vault = 1000. The member's payout is 100, so their share is 0.1.
  const me = computeUserHuntStats(hs, 'member3');
  assert.strictEqual(me.usd.returned, 100);
  assert.strictEqual(me.usd.vault, 60);       // 10% of the 600 vault, NOT the whole 600
  assert.strictEqual(me.usd.vaultHunts, 1);
  // The host still owns the whole hunt, so their vault figure is unchanged by the split.
  const host = computeUserHuntStats(hs, 'host3');
  assert.strictEqual(host.usd.vault, 600);
});
