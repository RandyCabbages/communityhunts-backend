...
test('a listed entry is KEPT even when the probe says dead — the listing decides removal', () => {
  // The avatarux-majestic-meow case: in the games API as type=slots/region_blocked=false and
  // genuinely unplayable (confirmed from Iowa, from Comoros, and on a live session). Policy is
  // that Rainbet's own listing wins anyway — a probe failure is one observation from one exit
  // point, and wrongly removing a live game is far more expensive than keeping a broken one.
  const live = index({
  ...
  assert.strictEqual(r.marked, 0, 'a listed entry must never be marked on probe evidence');
  assert.strictEqual(r.swept, 0);
  assert.strictEqual(r.entries[0].missingSince, undefined);
  assert.deepStrictEqual(r.advisoryDead, ['avatarux-majestic-meow'], 'but it is reported');
});

test('a listed entry with an old stamp is un-marked even when the probe says dead', () => {
  // Guards the transition: entries stamped under the old "dead overrides listing" rule must
  // be cleared rather than left to age into a sweep.
  const live = index({
    slugs: new Set(['avatarux:majesticmeow']),
    reachableProviders: new Set(['avatarux']),
  });
  const entries = [{ name: 'Majestic Meow', rainbetSlug: 'avatarux-majestic-meow', missingSince: '2026-07-01' }];
  const r = reconcile(entries, live, opts({
    playability: new Map([['avatarux-majestic-meow', 'dead']]),
  }));
  assert.strictEqual(r.swept, 0);
...
test('a dead verdict on an UNLISTED entry still confirms the removal', () => {
  const entries = [{ name: 'Gone', rainbetSlug: 'hacksaw-gone', missingSince: '2026-07-20' }];
  const r = reconcile(entries, index(), opts({
    playability: new Map([['hacksaw-gone', 'dead']]),
  }));
  assert.strictEqual(r.swept, 1);
  assert.deepStrictEqual(r.advisoryDead, [], 'not advisory — this one was genuinely delisted');
});
