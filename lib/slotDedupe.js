...
function planDedupe(entries, liveByNameKey) {
  ...
  for (const [k, rows] of groups) {
    ...
    // Several studios claim this name. If Rainbet lists the name at all, buckets it
    // does NOT serve are mis-attributed provider prefixes for a game already kept
    // under its real slug — drop those. If Rainbet lists none of them, we cannot
    // arbitrate, so keep everything (deciding it is dead is reconcile's call).
    const matched = bucketWinners.filter(b => liveCanons.has(b.canon));
    if (matched.length) {
      keep.push(...matched.map(b => b.winner));
      const unmatched = bucketWinners.filter(b => !liveCanons.has(b.canon));
      drop.push(...unmatched.map(b => b.winner));
      report.push({
        key: k,
        action: matched.length === bucketWinners.length ? 'kept-distinct' : 'cross-collapse',
        kept: matched.map(b => b.winner.rainbetSlug),
        dropped: unmatched.map(b => b.winner.rainbetSlug),
      });
      if (matched.length === bucketWinners.length) distinctKept += matched.length;
    } else {
