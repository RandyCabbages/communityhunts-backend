...
function guildFlags(src) {
  ...
  // Plain guild presence, distinct from any role. It verifies a self-join for the Partner
  // extension perk (lib/features.js), and it rides along here rather than being looked up live
  // because reqHasFullExtension runs on every extension load and must stay I/O-free.
  if (src && 'isGuildMember' in src) f.isGuildMember = !!src.isGuildMember;
  return f;
}
...
// Gate for the BOARD routes of a shared singleton hunt (affiliate / VIP): the mod crew, plus
// anyone a mod has invited to co-run this particular hunt. `keyFn(tenantId)` names the slot
// (affiliateHuntKey / vipHuntKey) so one factory serves both surfaces.
//
// Editors live in `boardEditors` and NOT in `invitedEditors` on purpose. canEditHunt reads
// invitedEditors, and it gates the generic /api/hunts/:userId/* family — a full hunt write that
// skips this router's sanitizers, plus invite/uninvite and share tokens. Putting a board-only
// helper in that field would have handed them all of it. Nothing reads both fields; adding a
// name here changes no existing gate.
//
// Invites are stored ON the hunt, so POST /reset clearing the slot also clears the list (that is
// the intended lifetime) and an empty slot admits mods only — there is no hunt to be invited to.
function requireBoardEditor(keyFn) {
  return function (req, res, next) {
    if (!req.user) return res.status(401).json({ error: 'Not authenticated' });
    if (reqIsMod(req)) return next();
    const hunt = hunts[keyFn((req.tenant && req.tenant.id) || 'bean')];
    // ID-only, String()-normalised on both sides — a display name must never match (repo rule),
    // and legacy/number-typed entries still compare equal to a real user.id.
    const editors = (hunt && hunt.boardEditors) || [];
    if (editors.some(e => String(e) === String(req.user.id))) return next();
