// Selection logic for GET /api/hall-of-fame: the all-time biggest hits that carry
// a watchable replay (mult >= FAME_MIN_MULT AND an http(s) replayUrl).
// Deliberately separate from /api/bangers, which is a recency rail (newest-first,
// 2-per-user diversity cap, 24-window) — under those rules an old record hit falls
// out. Here: multiplier-descending, NO per-user cap (records are records).
// Returns the FULL eligible list — truncation belongs to the caller: the hub route
// slices to FAME_CAP, the /all route pages with limit/offset (routes/misc.routes.js).
// replayUrl is sanitized at write time (sanitizeBonusReplayUrls, lib/hunts-core.js);
// the regex check here is defense-in-depth for pre-sanitizer rows.

// Shared-hunt runner attribution — see the note at the huntRunnerOf call below. Pure reads over a
// hunt object; hunts-core requires nothing from here, so there is no cycle.
const { huntRunnerOf, huntCategoryOf } = require('./hunts-core');

const FAME_MIN_MULT = 300;
const FAME_CAP = 12;
const FAME_PAGE_DEFAULT = 24;
const FAME_PAGE_MAX = 50;

function collectHallOfFame(hunts, archive, tenantId, { isAnon = () => false } = {}) {
  const out = [], seen = new Set();
  const tid = tenantId || 'bean';
  const collect = (h, live) => {
    if (!h || !h.user || !Array.isArray(h.bonuses)) return;
    if ((h.tenantId || 'bean') !== tid) return;
    const at = h.archivedAt || h.startedAt || null;
    for (const b of h.bonuses) {
      const bet = +b.bet || 0, win = +b.win || 0;
      if (bet <= 0 || win <= 0) continue;
      const mult = win / bet;
      if (mult < FAME_MIN_MULT) continue;
      const replayUrl = typeof b.replayUrl === 'string' ? b.replayUrl.trim() : '';
      if (!/^https?:\/\//i.test(replayUrl)) continue;
      const key = `${h.user.id}|${(b.slot || '').toLowerCase()}|${bet}|${win}`;
      if (seen.has(key)) continue;
      seen.add(key);
      const anon = isAnon({ discordId: h.user.id, name: h.user.displayName });
      // A shared VIP/Affiliate hunt is owned by the COMMUNITY, so `user.displayName` is the tenant
      // host ("Bean") whoever actually ran it. Same resolution huntSummary and lib/bangers.js use;
      // null for every other hunt, whose owner name is already correct. Name and avatar come from
      // the SAME source so a runner never inherits the host's face.
      const runner = huntRunnerOf(h, isAnon);
      out.push({
        slot: b.slot || 'Unknown', bet, win, mult: +mult.toFixed(2),
        // Legacy hunts predate currency tracking; USD is the same fallback used by
        // hunts-core.js and the frontend's `hunt.currency || 'USD'`.
        currency: h.currency || 'USD',
        userId: h.user.id,
        username: runner ? runner.name : (anon ? 'Anonymous' : h.user.displayName),
        avatar: runner ? runner.avatar : (anon ? null : h.user.avatar),
        // huntType cannot tell Affiliate from VIP (emptyAffiliateHunt stamps huntType 'vip');
        // `category` is derived from the hunt key, as huntSummary sends to the hub.
        huntType: h.huntType || 'community', category: huntCategoryOf(h), live: !!live,
        at, archivedAt: h.archivedAt || null, replayUrl,
      });
    }
  };
  // Live hunts first so their fresher copy wins the dedupe over an archived snapshot.
  Object.values(hunts).forEach(h => { if (h.isLive) collect(h, true); });
  archive.forEach(h => collect(h, false));
  out.sort((a, b) => {
    if (b.mult !== a.mult) return b.mult - a.mult;
    const ta = a.at ? new Date(a.at).getTime() : 0;
    const tb = b.at ? new Date(b.at).getTime() : 0;
    return tb - ta; // tiebreak: newer first
  });
  return out;
}

// Page a collectHallOfFame() list for GET /api/hall-of-fame/all. Inputs are untrusted
// query-string values, so every bad shape (garbage, NaN, negative, 0, object) falls back
// to the default rather than erroring — that contract now covers `currency` and `sort` too.
// `total` is the FILTERED count (the client needs it for "N more" and to know when to stop);
// `currencies` is the rollup over the FULL set, because the tab bar must show every tab's
// count while one tab is selected. Never mutates `list` — the caller's array is the shared
// selection result.
function pageHallOfFame(list, { limit, offset, currency, sort } = {}) {
  const int = (v) => {
    const n = typeof v === 'string' || typeof v === 'number' ? Math.floor(Number(v)) : NaN;
    return Number.isFinite(n) ? n : NaN;
  };
  const l = int(limit);
  const o = int(offset);
  const safeLimit = Number.isFinite(l) && l >= 1 ? Math.min(l, FAME_PAGE_MAX) : FAME_PAGE_DEFAULT;
  const safeOffset = Number.isFinite(o) && o >= 0 ? o : 0;

  const codeOf = (h) => h.currency || 'USD';

  // Rollup over the FULL set. Tiebreak on code so equal counts have a stable order.
  const counts = new Map();
  for (const h of list) counts.set(codeOf(h), (counts.get(codeOf(h)) || 0) + 1);
  const currencies = [...counts.entries()]
    .map(([code, total]) => ({ code, total }))
    .sort((a, b) => (b.total - a.total) || a.code.localeCompare(b.code));

  // Only filter on a code that actually exists in this set; anything else is unfiltered.
  const want = typeof currency === 'string' ? currency.toUpperCase() : '';
  const filtered = counts.has(want) ? list.filter(h => codeOf(h) === want) : list;

  // `list` arrives multiplier-desc from collectHallOfFame; only 'latest' re-sorts it.
  const ts = (h) => (h.at ? new Date(h.at).getTime() : 0);
  const ordered = String(sort || '').toLowerCase() === 'latest'
    ? [...filtered].sort((a, b) => (ts(b) - ts(a)) || (b.mult - a.mult))
    : filtered;

  return {
    items: ordered.slice(safeOffset, safeOffset + safeLimit),
    total: ordered.length,
    offset: safeOffset,
    limit: safeLimit,
    currencies,
  };
}

module.exports = {
  collectHallOfFame, pageHallOfFame,
  FAME_MIN_MULT, FAME_CAP, FAME_PAGE_DEFAULT, FAME_PAGE_MAX,
};
